...
def plotHeapUsage(mode, appName, title, gclogFile, outputFile):
    ...
    axes[0].set_ylabel("Old Gen (MB)")
    axes[1].set_ylabel("GC time (s)")

    axes[0].set_ylim(0, 7000)  # The ceil
    ...
    FGCBar = axes[1].bar(fgcTime, fgcPause, 0.01, color=colors2[2], label="FGC Pause", edgecolor=colors2[2])
    YGCBar = axes[1].bar(ygcTime, ygcPause, 0.01, color=colors2[4], label="YGC Pause", edgecolor=colors2[4])
