...
def plotHeapUsage(timeOffset, cpuTimeOffset, mode, appName, title, gclogFile, topMetricsFile, outputFile):

    heapUsage = HeapUsage()
    heapUsage.initHeapUsage(gclogFile, timeOffset)

    fig, axes = plt.subplots(nrows=3, ncols=1, sharey=False, sharex= True, figsize=(4.3,4.5))
    ...
    #     axes[1].plot(-1000,-1000, 'bo',markersize=4,label='Concurrent sweep phase')
    ...
    if (topMetricsFile != ""):
        ...
        axes[2].legend(markerfirst=False,frameon=False, labelspacing=0.2,
                       ncol=1, borderaxespad=0.4, columnspacing=1.2, handletextpad=0.5, loc="upper right")
        ax12.tick_params('y', colors='b')
        ax12.set_ylim(0, 7.8)  # The ceil
...
if __name__ == '__main__':
    ...
    cpuTimeOffset = parallelTimeOffset
    plotHeapUsage(parallelTimeOffset, cpuTimeOffset,
                  mode, appName, "(a) PageRank-0.5-Parallel-iterative-tasks",
                  inputFile + "Parallel/Parallel-E" + str(parallelExecutorID) + "-parsed.txt",
                  inputFile + "Parallel/topMetrics.txt",
                  inputFile + "Parallel/Parallel-E" + str(parallelExecutorID) + ".pdf")
    ...
    #plotHeapUsage(CMSTimeOffset, mode, appName, "(b) PageRank-0.5-CMS-iterative-tasks", inputFile + "CMS/CMS-E" + str(cmsExecutorID) + "-parsed.txt", inputFile + "CMS/CMS-E" + str(cmsExecutorID) + ".pdf")
