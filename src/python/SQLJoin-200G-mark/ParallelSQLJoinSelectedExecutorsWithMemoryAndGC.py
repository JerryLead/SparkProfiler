...
def plotHeapUsage(collectorFile, heapTimeOffset, cpuTimeOffset, mode, appName, title, originalLogFile, gclogFile, topMetricsFile, outputFile):
    ...
    axes[1].set_ylim(0, 0.26) #maxPause * 1.35)
    ...
    if topMetricsFile != "":
        ...
        axes[2].set_ylim(0, 380) #800)  # The ceil
        ...
        axes[2].legend(markerfirst=False,frameon=False, borderaxespad=0.03, labelspacing=0.1, loc ='upper left')
        ...
        if file.startswith("Parallel"):
            for executor in os.listdir(os.path.join(inputFile, file)):
                if executor.startswith("E"):
                    plotHeapUsage(file, 1365, 1369, mode, appName, "(a) SQLJoin-1.0-Slowest-Parallel-Task",
